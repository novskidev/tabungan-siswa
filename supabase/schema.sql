...
-- Last synced: teacher multi-class 20250905000000_teacher_multi_class.sql.
...
-- Teacher can hold several classes ('1'-'6'); empty until master assigns. Master sees all.
...
-- ---------- master RPCs ----------
-- list_teachers(): master-only teacher list (SECURITY DEFINER, incl classes).
-- create_teacher_profile(uuid, text, text[]): master-only teacher insert.
-- add_teacher_class(uuid, text) / remove_teacher_class(uuid, text): master-only
-- class assignment (one teacher can hold several classes).
